...
其中关于 `move` 这个 API 看到两个版本：一个是没有 parameter，每次就朝机器人面向的方向前进一步，
所以需要自己在递归中维护方向；一个是可以传 direction 进去，让机器人直接朝那个方向走一步。
...
实现肯定有很多种，但我倾向把 Room 和 Robot 解耦。因为其实做到后来你会发现，
Robot 根本不需要知道他在 Room 的实际座标，也不需要知道在 Room 的相对方位，
在搜索的过程中维护一套相对的就行（我觉得还挺 make sense 的，毕竟我们在路上走也不需要知道实际经纬度和实际方位）。
...
这样做的好处是要转换方位只需要 `(i + k) % 4` 就行。python 里面能直接 `(i - k) % 4`，也可以直接 `(i - k + 4) % 4` 先换成正数。
...
# down
复制代码
大白话就是，如果下方 (D) 没去过，而且没墙，就去 (DFS)，回来之后 (机器人面向上方) 转右边进去右方 (R)；
如果不能去下方，那么 (机器人面向下方) 转左边进去右方。
- 出格子：要让递归返回的时候，Robot 刚好朝向进去格子的反方向(用前述十字形的移动)，
